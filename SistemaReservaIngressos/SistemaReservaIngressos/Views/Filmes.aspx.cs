            ...
            try
            {
                using (HttpClient httpClient = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        foreach (var filme in filmes)
                        {
                            ...
                            ImageButton imgButton = new ImageButton
                            {
                                ...
                                Height = 250,
                                Width = 181,
                                CommandArgument = filme["filmeId"].ToString() 
                            };
                            imgButton.Click += new ImageClickEventHandler(btnCompraClick);
                            ...
                            divContainer.Controls.Add(imgButton); 
                            divContainer.Controls.Add(new Literal { Text = "<br />" }); 
                            divContainer.Controls.Add(lblFilme);
                    ...
                    else
                    {
                        ...
                        filmesContainer.Controls.Add(lblErro);
                    }
                }
            }
            catch (Exception ex)
        ...
        protected async void btnCompraClick(object sender, ImageClickEventArgs e) 
        {
            ...
            {   //Converte o objeto sender para um ImageButton para permitir o acesso a suas propriedades
                ...
                using (HttpClient client = new HttpClient()) 
                {
                    ...
                    if (response.IsSuccessStatusCode) 
                    {
                        Filme filme = await response.Content.ReadAsAsync<Filme>(); 
                        string detalhesUrl = $"~/Views/ComprarFilme.aspx?id={filmeId}";
                        Response.Redirect(detalhesUrl); 
                    }
                    else
                    {
